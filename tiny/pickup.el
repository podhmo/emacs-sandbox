;;; pickup.el --- rfind -*- lexical-binding: t; -*-
...
(cl-defsubst %pickup:string-strip-left (string &optional (pat-arg "[ \t]"))
  (replace-regexp-in-string (format "^%s+" pat-arg) "" string))

(cl-defsubst %pickup:string-strip-right (string &optional (pat-arg "[ \t]"))
  (replace-regexp-in-string (format "%s+$" pat-arg) "" string))
...
;; produce, consume
(cl-defun pickup:produce-parent-canidates (path &key (delimiter "/"))
  "a/b/c -> (a/b/c a/b a)"
  (cl-destructuring-bind (head . rest) (split-string path delimiter)
...
(defun pickup:consume-find-matched-file (candidates target)
  (let ((target (%pickup:string-strip-left target "/")))
    (cl-find target candidates :test '%pickup:file-exists-p)))
...
(cl-defun pickup:pickup-parent (file &key path produce candidates)
  (let ((candidates (or candidates (pickup:get-candidates produce path))))
    (pickup:consume-find-matched-file candidates file)))

(cl-defun pickup:pickup-file (file &key path produce candidates)
...
(cl-defun pickup:pickup-parent-with-config-file (file &key path produce candidates config)
...
(cl-defun pickup:pickup-file-with-config-file (file &key path produce candidates config)
  (let* ((config-name (or config pickup:reference-path-config-filename))
         (config-path (pickup:pickup-file
                       config-name :path path :produce produce :candidates candidates)))
    (and config-path
...
;; toplevel
(cl-defun pickup-parent (file &key path produce)
...
;;;###autoload
(cl-defun pickup-file (file &key path produce)
  (interactive "sfile: ")
  (let ((candidates (pickup:get-candidates produce path)))
